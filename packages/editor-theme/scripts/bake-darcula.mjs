 ...
 *
 * Philosophy (decision B + 2-lobe): Darcula's HUES are its signature and are LOCKED.
 * Every token's LIGHTNESS is re-solved through Delta's two-lobe Helmholtz-Kohlrausch
 * apparent-lightness model so that all tokens on one salience tier read as equally
 * bright to the eye — including the sweet/dotprompt tokens JetBrains never had a hue
 * for. The math is run ONCE here and frozen to hex; the editor pays zero runtime cost
